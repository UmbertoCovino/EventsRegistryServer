...
public class EventsFromDateToDateJSON extends ServerResource {
	...
	public String getEvents() throws ParseException, InvalidEventIdException, InvalidUserEmailException {   	
		...
		try {
			Date fromDate = Event.DATETIME_SDF.parse(URLDecoder.decode(getAttribute("from"), "UTF-8"));
			Date toDate = Event.DATETIME_SDF.parse(URLDecoder.decode(getAttribute("to"), "UTF-8"));
